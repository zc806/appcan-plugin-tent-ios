...
- (BOOL) publishWeibo:(NSString *)weiboContent jing:(NSString *)jing wei:(NSString *)wei format:(NSString *)format clientip:(NSString *)clientip syncflag:(NSString *)syncflag {
   ...
   // [opaqueview setHidden:YES];
    ...
    if (self.retCode == resSuccessed) {
		...
		NSDictionary *dict =(NSDictionary*)[resultStr objectFromJSONString];
		//[OpenSdkBase showMessageBox:resultStr]; 
		//NSLog(@"dict=%@",[dict description]);
		NSNumber *retNum =[dict objectForKey:@"ret"];
		if ([retNum isEqualToNumber:[NSNumber numberWithInt:0]]) {
			return YES;
		}
		return NO;
    }
...
-(BOOL)publishWeiboWithImage:(NSString *)filePath weiboContent:(NSString *)weiboContent jing:(NSString *)jing wei:(NSString *)wei format:(NSString *)format clientip:(NSString *)clientip syncflag:(NSString *)syncflag {
